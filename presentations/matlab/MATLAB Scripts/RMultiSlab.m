...
rSlab_TE = [];
rSlab_TM = [];
sinTh(M+2) = sin(Theta_inc); % sin(Theta) in the incidence medium

% Calculate wave number (k) for each medium
for i = 1:M+2
    k(i) = 2 * pi * f * sqrt(eps(i) * mu(i)); % Wave number in medium i
end
...
rSlab_TE(1) = -1; % TE polarization
rSlab_TM(1) = -1; % TM polarization
