%This code is used to produce an optimized 4 layer RAM using GA
...
dataFile = 'Total_Thickness_VS_Reflection_4Layer.mat';
...
nVar = 4; % Number of layers (updated from 3 to 4)
...
% Setting IntCon constraint so materials and thickness don't mix (5 6 7 8)
% are thickness indices
IntCon = [5 6 7 8]; % Updated to include the fourth layer
...
[opt_params, fval] = ga(@(params) reflectionObjective(params, f, M_epsr, M_mur), ...
    2 * nVar, [], [], [], [], lb, ub, ...
    @(params) combinedConstraint(params), IntCon, options);
...
fileID = fopen('Optimized_Structure_4Layer.txt', 'a'); % Open in append mode
...
fclose(fileID);
disp('Structure appended to Optimized_Structure_4Layer.txt');
...
title('Total Thickness vs. Minimum Reflection');
grid on;

% Plot the optimized reflection curve
figure;
semilogx(f, rSlab_TE_db, 'r', 'LineWidth', 2.5);
title('Optimized 4 Layered RAM Structure Reflection');
...
function [c, ceq] = combinedConstraint(params)
    % Thickness constraint
    [c1, ceq1] = thicknessConstraint(params);
    
    % No consecutive layers constraint
    [c2, ceq2] = noConsecutiveLayersConstraint(params);
...
%% No Consecutive Layers Constraint Function
function [c, ceq] = noConsecutiveLayersConstraint(params)
    % Extract material selection values
    nVar = length(params) / 2;
    material_selection = round(params(nVar+1:end));

    % Check for consecutive identical materials
    c = 0; % Initialize constraint value
    for i = 1:nVar - 1
        if material_selection(i) == material_selection(i + 1)
            c = c + 1; % Increment the constraint value if consecutive materials are the same
        end
    end
    
    % No equality constraints for this function
    ceq = [];
end
