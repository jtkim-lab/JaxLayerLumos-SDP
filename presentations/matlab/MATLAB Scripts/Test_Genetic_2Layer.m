%This code is used to produce an optimized 2 layer RAM using GA
...
dataFile = 'Total_Thickness_VS_Reflection_2Layer.mat';
...
nVar = 2; % Number of layers (updated from 4 to 2)
...
% Setting IntCon constraint so materials and thickness don't mix (3 4)
% are thickness indices
IntCon = [3 4]; % Updated to include only two layers
...
fileID = fopen('Optimized_Structure_2Layer.txt', 'a'); % Open in append mode
...
fclose(fileID);
disp('Structure appended to Optimized_Structure_2Layer.txt');
...
title('Total Thickness vs. Minimum Reflection for 2 Layer Structure');
grid on;

% Plot the optimized reflection curve
figure;
semilogx(f, rSlab_TE_db, 'r', 'LineWidth', 2.5);
title('Optimized 2 Layered RAM Structure Reflection');
