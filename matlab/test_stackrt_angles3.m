...
coeff_TE = zeros(length(theta_inc), 2, length(frequencies), length(materials)+1);
coeff_TM = zeros(length(theta_inc), 2, length(frequencies), length(materials)+1);
...
for i = 1:length(theta_inc)
  ...
  [aSlab_TM_abs] = calc_absorption_from_coeffs(thickness_materials, kz, coeff_TE);
end
