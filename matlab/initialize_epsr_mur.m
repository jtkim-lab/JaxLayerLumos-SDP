function [epsr, mur, M_epsr, M_mur] = initialize_epsr_mur(materialInd, f)
...
% Initialize epsr and mur for infinite medium and air
epsr = [ones(1, length(f)); M_epsr(materialInd, :); ones(1, length(f))];
mur = [ones(1, length(f)); M_mur(materialInd, :); ones(1, length(f))];